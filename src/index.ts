	...
	onload(): void {
		...
		this.addCommand({
			...
			callback: () => {
				console.log('------')
				// 设置 css 变量
				const currentGap = parseInt(getComputedStyle(root).getPropertyValue('--custom-gap').replace('px', '')) || 4
				const newGap = Math.max(2, currentGap - 1) // 防止过小
		...
		this.addCommand({
			...
			callback: () => {
				...
				// const root = document.documentElement
				const currentGap = parseInt(getComputedStyle(root).getPropertyValue('--custom-gap').replace('px', '')) || 4
				const newGap = Math.max(10, currentGap + 1) // 防止过大
				root.style.setProperty('--custom-gap', newGap.toString() + 'px')
			},
		})
		this.addCommand({
			...
			callback: () => {
				...
				const currentLineHeight = parseFloat(parseFloat(getComputedStyle(root).getPropertyValue('--custom-line-height')).toFixed(2)) || 1
				const newLineHeight = Math.max(0.5, currentLineHeight - 0.1) // 防止过小
		...
		this.addCommand({
			...
			callback: () => {
				...
				const currentLineHeight = parseFloat(parseFloat(getComputedStyle(root).getPropertyValue('--custom-line-height')).toFixed(2)) || 1
				const newLineHeight = Math.max(0.5, currentLineHeight + 0.1) // 防止过大
				root.style.setProperty('--custom-line-height', newLineHeight.toString())
			},
		})
	}
